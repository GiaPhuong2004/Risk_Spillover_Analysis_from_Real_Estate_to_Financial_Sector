...
## 1. Xử lí dữ liệu đầu vào
### 1.1. Nhập dữ liệu
```{r}
# Danh sách file path (mỗi file là một ngành)
file_paths <- list(
  "D:/Khoa_luan/Dữ liệu Lịch sử Real Estate.xlsx",
  "D:/Khoa_luan/Dữ liệu Lịch sử Banks.xlsx",
  "D:/Khoa_luan/Dữ liệu Lịch sử Financial Services.xlsx",
  "D:/Khoa_luan/Dữ liệu Lịch sử Insurance.xlsx"
)

# Hàm chuyển đổi giá trị KL
...
# Khởi tạo danh sách để lưu dữ liệu từ các file
merged_cp_list <- list()
merged_kl_list <- list()
...
for (file_path in file_paths) {
  ...
  # Tên ngành từ file path (Real Estate, Banks, Financial Services, Insurance)
  industry_name <- str_extract(file_path, "(?<=Dữ liệu Lịch sử ).*(?=\\.xlsx)")
  
  # Xử lý giá đóng cửa
  data_cp <- map(sheet_names, function(sheet) {
    stock_code <- sub("Dữ liệu Lịch sử ", "", sheet)
    
    read_excel(file_path, sheet = sheet) %>%
      select(Ngày, `Lần cuối`) %>%
      mutate(Date = as.Date(Ngày, format = "%d/%m/%Y")) %>%
      select(Date, `Lần cuối`) %>%
      rename(!!stock_code := `Lần cuối`)
  })
  
  # Gộp theo Date cho giá đóng cửa
  merged_cp <- purrr::reduce(data_cp, full_join, by = "Date") %>%
    arrange(Date)
  
  # Lưu DataFrame giá đóng cửa cho ngành
  merged_cp_list[[industry_name]] <- merged_cp
  
  # Xử lý khối lượng giao dịch
  ...
  # Gộp theo Date cho khối lượng giao dịch
  merged_kl <- purrr::reduce(data_kl, full_join, by = "Date") %>%
    arrange(Date)
  
  # Chuyển đổi giá trị KL
  merged_kl <- merged_kl %>%
    mutate(across(-Date, ~ sapply(.x, convert_value)))
  
  # Lưu DataFrame khối lượng giao dịch cho ngành
  merged_kl_list[[industry_name]] <- merged_kl
}

# Giờ có 8 DataFrame riêng biệt cho từng ngành

# In kết quả cho từng DataFrame (giá đóng cửa và khối lượng giao dịch)
print(head(merged_cp_list[["Real Estate"]]))  # Giá đóng cửa Real Estate
print(head(merged_kl_list[["Real Estate"]]))  # Khối lượng giao dịch Real Estate

print(head(merged_cp_list[["Banks"]]))        # Giá đóng cửa Banks
print(head(merged_kl_list[["Banks"]]))        # Khối lượng giao dịch Banks

print(head(merged_cp_list[["Financial Services"]]))  # Giá đóng cửa Financial Services
print(head(merged_kl_list[["Financial Services"]]))  # Khối lượng giao dịch Financial Services

print(head(merged_cp_list[["Insurance"]]))  # Giá đóng cửa Insurance
print(head(merged_kl_list[["Insurance"]]))  # Khối lượng giao dịch Insurance
```

### 1.2. Tính giá đóng cửa và lợi suất từng ngành
```{r}
# Tính giá đóng cửa ngành theo KL cho từng ngành
industry_avg_cp <- map2(merged_cp_list, merged_kl_list, function(cp_df, kl_df) {
  # Thay NA bằng 0 trước khi tính
  cp_filled <- cp_df %>% mutate(across(-Date, ~ replace_na(.x, 0)))
  kl_filled <- kl_df %>% mutate(across(-Date, ~ replace_na(.x, 0)))
  
  # Tính giá trị: sum(cp * kl) / sum(kl)
...
# Đặt tên cho các ngành trong danh sách
names(industry_avg_cp) <- names(merged_cp_list)

# Đổi tên cột giá đóng cửa ngành theo tên ngành
industry_avg_cp_named <- imap(industry_avg_cp, ~ rename(.x, !!.y := Avg_Closing_Price))

# Gộp lại thành 1 bảng duy nhất theo Date
...
```{r}
# --- Tính giá đóng cửa trung bình của ngành Finance (Banks + Financial Services + Insurance) ---

# Lấy dữ liệu giá và KL của 3 ngành tài chính
...
# Gộp toàn bộ cổ phiếu trong 3 ngành thành 1 bảng lớn
...
# Tính giá đóng cửa trung bình có trọng số: sum(cp * kl) / sum(kl)
...
# Gộp thêm vào industry_cp_combined
...
```
```{r}
#Tính return rate theo công thức log(Pt / Pt-1) * 100
industry_return <- industry_closing_price %>%
  arrange(Date) %>%  # Đảm bảo dữ liệu theo thứ tự thời gian
  mutate(
...
# Vòng lặp vẽ từng ngành
for (ind in industries) {
  p <- ggplot(industry_closing_price, aes_string(x = "Date", y = ind)) +
    geom_line(color = "steelblue") +
    labs(
      title = paste("Giá đóng cửa ngành", ind, "2018-2024"),
      x = "Ngày",
      y = "Giá đóng cửa"
    ) +
    theme_minimal() +
...
for (ind in industries) {
  p <- ggplot(industry_return, aes_string(x = "Date", y = ind)) +
    geom_line(color = "steelblue") +
    labs(
      title = paste("Lợi suất ngành", ind, "2018-2024"),
      x = "Ngày",
      y = "Lợi suất"
    ) +
    theme_minimal() +
...
```{r}
# Vòng lặp vẽ histogram + density cho từng ngành
for (ind in industries) {
  p <- ggplot(industry_return, aes_string(x = ind)) +
    geom_histogram(aes(y = ..density..),   # histogram theo mật độ
                   bins = 40,            # số cột
                   fill = "steelblue",
                   color = "white",
                   alpha = 0.7) +
    geom_density(aes(y = ..density..),     # phủ đường density
                 color = "red",
                 size = 1) +
    labs(
      title = paste("Phân phối lợi suất ngành", ind, "2018–2024"),
      x = "Lợi suất",
      y = "Tần suất"
    ) +
    theme_minimal() +
...
## 2.Mô hình ARIMA 
### 2.1. Kiểm định JB và tính dừng 
```{r}
#Kiểm định JB cho từng ngành
...
for (ind in industries) {
  cat("\n============================\n")
  cat("Kiểm định Ljung–Box cho ngành:", ind, "\n")
  cat("============================\n\n")
  
  series <- na.omit(industry_return)[[ind]]
  
  # Kiểm định Ljung-Box ở nhiều độ trễ (lag)
  for (lag in c(1, 5, 10)) {
...
### 2.3. Chạy mô hình ARIMA(p,d,q)
```{r}
# Tạo list rỗng để lưu kết quả mô hình
arma_models <- list()

# Lặp qua từng ngành
for (ind in industries) {
  cat("🔹 Đang chạy auto.arima cho ngành:", ind, "\n")
  
  y <- industry_return[[ind]]
  
  # Chạy auto.arima
  fit <- auto.arima(y,
                    d = 0,            # returns thường đã dừng
                    seasonal = FALSE, 
                    ic = "aic",      
                    stepwise = FALSE, 
                    approximation = FALSE)
  
  # Lưu mô hình vào list
  arma_models[[ind]] <- fit
...
```{r}
#Kiểm định Ljung-Box
# Fit ARIMA(2,0,4) cho tất cả các chuỗi
fixed_arima_models <- list()

for (ind in industries) {
  cat("Fitting ARIMA(2,0,4) for ngành:", ind, "\n")
...
for (ind in names(fixed_arima_models)) {
  ...
  for (lag_val in lags_to_check) {
    ...
    if (lb$p.value < 0.05) {
      cat(" ⚠️  Phần dư còn tự tương quan.\n")
    } else {
      cat(" ✅  Phần dư không có tự tương quan đáng kể.\n")
    }
  }
  cat("------------------------------------------------------------\n")
}
...
for (ind in names(fixed_arima_models)) {
  ...
  for (lag_val in lags_to_check) {
    ...
    if (arch_test$p.value < 0.05) {
      cat("  ⚠️  Có ARCH effect → mô hình ARIMA(2,0,4) KHÔNG đủ, nên xem xét GARCH.\n")
    } else {
      cat("  ✅  Không có ARCH effect đáng kể.\n")
    }
  }
  cat("------------------------------------------------------------\n")
}
...
#-------------------------
# ARMA-GARCH cho nhiều ngành & phân phối
#-------------------------

library(rugarch)
library(tseries)   # cho ArchTest

# Danh sách phân phối thử
...
# Vòng lặp qua ngành (bỏ cột Date)
for (ind in names(industry_return)[names(industry_return) != "Date"]) {
  
  ret <- na.omit(as.numeric(industry_return[[ind]]))  # chỉ lấy giá trị số
  
  # Lấy order ARMA từng ngành
  p <- 2
  q <- 4
  
  cat("🔹 Ngành:", ind, "\n")
  
  # Vòng lặp qua phân phối
  for (dist in dist_list) {
    ...
    if (!is.null(garch_fit)) {
      ...
      cat("  ✅ Fit thành công cho", dist, "\n")
      
      # Kiểm định Ljung-Box trên phần dư chuẩn hóa
...
```{r}
#--------------------------------------------------------
# ARMA(2,4) – EGARCH(1,1) với xử lý lỗi hội tụ
#--------------------------------------------------------
...
for (ind in names(industry_return)[names(industry_return) != "Date"]) {
  ...
  for (dist in dist_list) {
    ...
    if (garch_fit@fit$convergence != 0) {
      cat("  ❌ Không hội tụ – bỏ mô hình", dist, "\n")
      next
    }
    ...
    if (is.null(ic) || any(is.na(ic))) {
      cat("  ❌ AIC/BIC bị NA – bỏ mô hình", dist, "\n")
      next
    }
    ...
    cat("  ✅ Fit thành công – lưu kết quả cho", dist, "\n")
    cat("------------------------------------------------------------\n")
  }
}
...
# Tham số ARMA cố định (ARMA(2,4))
p <- 2
q <- 4

# Vòng lặp qua các ngành
for (ind in names(industry_return)[names(industry_return) != "Date"]) {
  ...
  for (dist in dist_list) {
    ...
    gjr_fit <- tryCatch({
      ...
      cat("  ⚠️ Không hội tụ – bỏ mô hình", dist, "\n")
      return(NULL)
    })
...
### 3.4. Chọn mô hình tối ưu
```{r}
#Chọn được mô hình GJR GARCH sged (3,2) 
#RealEstate
...
```{r}
# =============================================================
#  Chuyển phần dư chuẩn hoá sang U(0,1)
# =============================================================
...
cat("Kolmogorov–Smirnov p-value (RealEstate):", round(ks_realestate$p.value, 4), "\n")
...
cat("Kolmogorov–Smirnov p-value (Banks):", round(ks_banks$p.value, 4), "\n")
...
cat("Kolmogorov–Smirnov p-value (FinancialServices):", round(ks_financialservices$p.value, 4), "\n")
...
cat("Kolmogorov–Smirnov p-value (Insurance):", round(ks_insurance$p.value, 4), "\n")
...
cat("Kolmogorov–Smirnov p-value (Finance):", round(ks_finance$p.value, 4), "\n")

# =============================================================
#  Kiểm tra trực quan (không bắt buộc)
# =============================================================
par(mfrow = c(1,3))
...
# =============================================================
#  Gộp lại thành ma trận U để dùng cho Copula
# =============================================================
...
```
```{r}
# Danh sách copula thử
copula_list <- c("normal", "t", "clayton", "gumbel", "frank")

# Danh sách cặp ngành muốn chạy copula
pairs_list <- list(
  c("RealEstate", "Banks"),
...
fit_dcc_pair <- function(series1, series2, name1, name2) {
  ...
  # Trả kết quả + mô hình để vẽ tương quan động
  return(list(result = result, model = dcc_fit))
}

# ==============================
# Bước 2. Chạy từng cặp
# ==============================
...
# ==============================
# Bước 4. Vẽ tương quan động
# ==============================
R_t_RE_Banks <- rcor(result_RE_Banks$model)   # ma trận tương quan động
corr_RE_Banks <- R_t_RE_Banks[1,2,]           # hệ số tương quan giữa 2 ngành
plot(corr_RE_Banks, type = "l", col = "blue",
     main = "Dynamic Correlation: RealEstate - Banks",
     ylab = "Correlation", xlab = "Time")

R_t_RE_FinancialServices <- rcor(result_RE_Financial$model)   # ma trận tương quan động
corr_RE_FinancialServices <- R_t_RE_FinancialServices[1,2,]           # hệ số tương quan giữa 2 ngành
plot(corr_RE_FinancialServices, type = "l", col = "blue",
     main = "Dynamic Correlation: RealEstate - FinancialServices",
     ylab = "Correlation", xlab = "Time")

R_t_RE_Insurance <- rcor(result_RE_Insurance$model)   # ma trận tương quan động
corr_RE_Insurance <- R_t_RE_Insurance[1,2,]           # hệ số tương quan giữa 2 ngành
plot(corr_RE_Insurance, type = "l", col = "blue",
     main = "Dynamic Correlation: RealEstate - Insurance",
     ylab = "Correlation", xlab = "Time")

R_t_RE_Finance <- rcor(result_RE_Finance$model)   # ma trận tương quan động
corr_RE_Finance <- R_t_RE_Finance[1,2,]           # hệ số tương quan giữa 2 ngành
...
## 6. Tính CoVaR các ngành tài chính
### 6.1. Tính CoVaR khi Real Estate ở trạng thái VaR
```{r}
alpha_level <- 0.05  # mức tin cậy 95%

# Hàm tính VaR theo ngành
compute_VaR_sged <- function(fit, alpha_level) {
  mu_i <- 0  # do mô hình trước dùng include.mean = FALSE
  sigma_t <- sigma(fit)    # volatility động theo thời điểm
...
# Tạo dataframe
VaR_df <- data.frame(
  Date = industry_return$Date[-1],  # bỏ NA đầu nếu cần
...
```{r}
#Hàm tính CoVaR
compute_CoVaR_TDCC_full <- function(u_i,         # uniform của sector i (conditioning) (0-1)
                                   u_j,         # uniform của sector j (target) (0-1)
                                   dcc_model,   # fitted DCC model object
                                   mu_j,        # conditional mean từ GARCH fit của j
                                   sigma_j,     # conditional sigma từ GARCH fit của j
                                   alpha_VaR,  # mức rủi ro
                                   alpha_CoVaR
                                   ) {
  
  n <- length(u_i)
  if(length(u_j)!=n || length(mu_j)!=n || length(sigma_j)!=n) stop("Các vector phải cùng chiều.")
  ...
  # ----- lấy degree of freedom (nu) từ DCC -----
  nu <- NA
  ...
  if(is.na(nu)) stop("Không tìm được nu trong dcc_model")
...
# ----- Step 4: map từ uniform -> quantile SGED -----
...
### 6.2. Tính CoVaR khi Real Estate ở trạng thái Median
```{r}
#Tính median cho từng ngành
alpha_level <- 0.5  # mức tin cậy 50%

# Hàm tính median theo ngành
...
```{r}
#Tính CoVaR tại khi Real Estate bình thường (median)
...
# ====================================
# Bước 1: Tính deltaCoVaR theo ngày
# ====================================
...
# Tính average annual deltaCoVaR 
